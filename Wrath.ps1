...
(gn "BtnDelay").Add_Click({   Show-Page $PageDelay })
(gn "BtnDebloat").Add_Click({ Show-Page $PageDebloat })
(gn "BtnGame").Add_Click({    Show-Page $PageGame })

(gn "BtnRestore").Add_Click({
    $r = [System.Windows.MessageBox]::Show(
        "Create a Windows restore point named 'Wrath Backup'?",
        "Wrath — Restore Point","YesNo","Question")
    if ($r -eq "Yes") {
        try {
            ...
            [System.Windows.MessageBox]::Show("Could not create restore point.`nEnsure System Restore is enabled in System Properties.","Wrath","OK","Warning")
        }
    }
})
...
(gn "DelayAll").Add_Click({   Set-SectionChecked "Delay" $true })
(gn "DelayRec").Add_Click({   Set-SectionChecked "Delay" $true -RecOnly })
(gn "DelayClear").Add_Click({ Set-SectionChecked "Delay" $false })
(gn "DelayBack").Add_Click({  Show-Page $PageHome })

# ════════════════════════════════════════════════════════
#  EVENTS — DEBLOAT PAGE
# ════════════════════════════════════════════════════════

(gn "DebloatAll").Add_Click({   Set-SectionChecked "Debloat" $true })
(gn "DebloatRec").Add_Click({   Set-SectionChecked "Debloat" $true -RecOnly })
(gn "DebloatClear").Add_Click({ Set-SectionChecked "Debloat" $false })
(gn "DebloatBack").Add_Click({  Show-Page $PageHome })

# ════════════════════════════════════════════════════════
#  EVENTS — GAME PAGE
# ════════════════════════════════════════════════════════

(gn "GameAll").Add_Click({   Set-SectionChecked "Game" $true })
(gn "GameRec").Add_Click({   Set-SectionChecked "Game" $true -RecOnly })
(gn "GameClear").Add_Click({ Set-SectionChecked "Game" $false })
(gn "GameBack").Add_Click({  Show-Page $PageHome })

# ════════════════════════════════════════════════════════
#  APPLY — LIVE EXECUTION
# ════════════════════════════════════════════════════════

function Append-Log($msg, $color = "#4a4a4a") {
...
$BtnApply.Add_Click({
    $selected = $Script:Checks | Where-Object { $_.Tag.Checked }
    ...
    $LogStatus.Text = "Applying $($selected.Count) tweak$(if($selected.Count -ne 1){'s'})..."

    # Run on UI thread with dispatcher yields so UI updates between tweaks
    $Window.Dispatcher.InvokeAsync({

        Append-Log "— Creating restore point before changes..." "#3a3a3a"
        try {
            Checkpoint-Computer -Description "Wrath Backup" -RestorePointType MODIFY_SETTINGS -EA Stop
            Append-Log "  Restore point created." "#2a5c3a"
        } catch {
            Append-Log "  Could not create restore point (System Restore may be off)." "#5c3a2a"
        }
        Append-Log ""
        ...
        foreach ($item in $selected) {
            $t = $item.Tag.Tweak
            $LogStatus.Text = "Applying: $($t.Name)"
            Append-Log "  $($t.Name)..." "#3a3a3a"
            $Window.Dispatcher.Invoke([System.Action]{}, "Background")
            try {
                & $t.Action
                Append-Log "  done." "#2a5c3a"
            } catch {
                Append-Log "  failed: $($_.Exception.Message)" "#5c2a2a"
            }
            $done++
        }

        Append-Log ""
        Append-Log "— All done. $done tweak$(if($done -ne 1){'s'}) applied." "#8b5cf6"
        Append-Log "  Restart your PC for all changes to take effect." "#3a3a3a"
        $LogStatus.Text = "Complete — $done tweak$(if($done -ne 1){'s'}) applied."
        $LogDone.Visibility = "Visible"

    }, [System.Windows.Threading.DispatcherPriority]::Background) | Out-Null
})

$LogDone.Add_Click({ Show-Page $PageHome })
